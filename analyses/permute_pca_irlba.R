...
expr_readcount_norm_log_corrected_scaled <-
    readRDS(file.path(data_directory,
...
summarize_permutations <- function(permutation_files_dir,
    ...
    pc_sdev_perm <-
        lapply(seq_len(num_replicates), function(i) {
            ...
            permutation_file <-
                file.path(permutation_files_dir,
                          paste0(permutation_files_prefix,
                          i,
                          ".rds"))
...
permutation_files_prefix <- paste0("prcomp_irlba_out_",
                                   "proportion",
                                   proportion_of_transcripts,
                                   "_n",
                                   n_use,
...
summarized_permutations <-
    summarize_permutations(permutation_files_dir = permutation_files_dir,
...
data.frame(orig = pc_var_explained,
    ...
    ggplot(aes(x = x,
               y = var_exp,
               group = category,
               color = category)) + geom_line() + geom_point() +
